...
module.exports = function badRequest(optionalData) {
  ...
  if (optionalData === undefined) {
    ...
    return res.status(statusCodeToSet).view(statusCodeToSet.toString());
  }
  ...
  // except in production use res.sendStatus().
    ...
    if (!_.isFunction(optionalData.toJSON)) {
      if (process.env.NODE_ENV === 'production') {
        ...
        return res.status(statusCodeToSet).send(optionalData.stack).view(statusCodeToSet.toString());
      }
    }
  }
  // Set status code and send response data.
  else {
    return res.status(statusCodeToSet).send(optionalData).view(statusCodeToSet.toString());
  }
